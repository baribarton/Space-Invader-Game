...
public class StartingClass extends Applet implements Runnable, KeyListener {
	...
	public static Spaceship spaceships, spaceships2, spaceships3, spaceship1, spaceship2, spaceship3, laser, laserDown,
	...
	private Image image, background, spaceshipDown, spaceshipRight, spaceshipLeft, laser1, laser2, laser3;
	...
	// game over states
	private Image currentGameOver, gameOver, yes, no;
	...
	private URL base;
	private Animation anim, rightShipAnim, downShipAnim, leftShipAnim, deathAnim;
	...
	public void paint(Graphics g) {
		...
		if (state == GameState.Running) {
			...
			for (int i = 0; i < rightLasers.size(); i++) {
				if (!rightEnemies.get(i).isMovingX()) {
					if (!rightEnemies.get(i).isMovingY()) {
						g.drawImage(laser2, (int) rightLasers.get(i).getCenterX() - 45,
								(int) rightLasers.get(i).getCenterY() + 25, this);
					}
				}
			}
			...
			for (int i = 0; i < leftLasers.size(); i++) {
				if (!leftEnemies.get(i).isMovingX()) {
					if (!leftEnemies.get(i).isMovingY()) {
						g.drawImage(laser3, (int) leftLasers.get(i).getCenterX() - 45,
								(int) leftLasers.get(i).getCenterY() + 25, this);
					}
				}
			}
			...
			// Right ship
			for (int i = 0; i < rightEnemies.size(); i++) {
				g.drawImage(rightShipAnim.getImage(), (int) rightEnemies.get(i).getCenterX() - 48,
						(int) rightEnemies.get(i).getCenterY() - 48, this);
			}

			// Left ship
			for (int i = 0; i < leftEnemies.size(); i++) {
				g.drawImage(leftShipAnim.getImage(), (int) leftEnemies.get(i).getCenterX() - 48,
						(int) leftEnemies.get(i).getCenterY() - 48, this);
			}
